...
class SearchBase extends React.Component {
  ...
  handleFilter(num) {
    ...
    if (num !== 0) {
      ...
      if (num % 3 === 1) filter.push([0, 3]);
      else if (num % 3 === 2) filter.push([4, 7]);
      else filter.push([8, 10]);
    }
    this.setState(() => ({
      filter: filter,
    }));
  ...
  renderSearchList(searchValue) {
    ...
    if (favorites != null) {
      ...
      favoritesRender = (
        <div>
          {favorites.map((value) => (
            <p className="center testAlign">
              <a
                className="accent"
                href={"http://" + base_url + "/plant/" + value}
                target="_blank"
      ...
      favoritesRender = (
        <p className="center testAlign">No Favorites are saved :(</p>
      );
    }
    return (
        ...
        <Row>
          ...
          <Col md={{ order: 3 }} className="addTop">
            <h5 className="center">
              ...
              >
                <i
                  aria-label="locationService"
                  className="fa fa-location-arrow primary"
                  id="locationService"
                ></i>
              </button>{" "}
            </h5>
          ...
          <Col md={{ order: 2 }} className="sb-wrapper">
            ...
            <Toast show={showTrefleDown} onClose={toggleShowTrefleDown}>
              ...
              <Toast.Body>
                Our data source is currently unavailable. Please try again
                later!
              </Toast.Body>
            </Toast>
          </Col>
  ...
  render() {
    ...
    return this.renderSearchList(searchValue);
  }
}
