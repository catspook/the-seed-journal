...
class PhBar extends React.Component {
  ...
  UNSAFE_componentWillMount() {
    if (
      typeof this.state.min !== "number" ||
      typeof this.state.max !== "number"
    )
      this.setState({
        valid: false,
      });
  ...
  renderInvalid() {
    return <span className="error accent">No pH Information Available</span>;
  }
  ...
  renderScale() {
    ...
    return (
      <div className="container chart ph-chart">
        ...
        <p>
          <b>Minimum pH</b> {this.state.min}, <b>Maximum pH</b> {this.state.max}
        </p>
      </div>
    );
  }
