...
class ClickToShow extends React.Component {
  ...
  handleClick() {
    this.setState(() => ({
      ...
      message:
        this.state.message === "click to show"
          ? "click to hide"
          : "click to show",
    }));
  }
  ...
  render() {
    ...
    return (
      <div className="click-wrapper">
        <h3 onClick={this.handleClick} className="h-click">
          <b>
            {this.props.header} <small>{this.state.message}</small>
          </b>
        </h3>
        {this.state.open &&
          this.state.category.map((key, index) => {
            return (
              <p key={index}>
                <b className="accent">{category[index] + " "}</b>
